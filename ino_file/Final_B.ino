...
void setup() {
  ...
  Firebase.setString(firebaseData,path + "/B1", "0");
  Firebase.setString(firebaseData,path + "/B2", "0");
}

void loop() {

  //장애물감지 센서부분
  data_B1 = digitalRead(0);
  data_B2 = digitalRead(2);
  ...
  if(old_data_B1 != data_B1){
    if(data_B1 == 0){
      Firebase.setString(firebaseData,path + "/B1", "0");
    }
    else if(data_B1 == 1){
      Firebase.setString(firebaseData,path + "/B1", "1");
    }
    old_data_B1 = data_B1;
  }

  if(old_data_B2 != data_B2){
    if(data_B2 == 0){
      Firebase.setString(firebaseData,path + "/B2", "0");
    }
    else if(data_B2 == 1){
      Firebase.setString(firebaseData,path + "/B2", "1");
    }
    old_data_B2 = data_B2;
  }
