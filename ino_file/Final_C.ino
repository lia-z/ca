...
void setup() {
  ...
  Firebase.setString(firebaseData,path + "/C1", "0");
  Firebase.setString(firebaseData,path + "/C2", "0");
}

void loop() {

  //장애물감지 센서부분
  data_C1 = digitalRead(0);
  data_C2 = digitalRead(2);
  ...
  if(old_data_C1 != data_C1){
    if(data_C1 == 0){
      Firebase.setString(firebaseData,path + "/C1", "0");
    }
    else if(data_C1 == 1){
      Firebase.setString(firebaseData,path + "/C1", "1");
    }
    old_data_C1 = data_C1;
  }

  if(old_data_C2 != data_C2){
    if(data_C2 == 0){
      Firebase.setString(firebaseData,path + "/C2", "0");
    }
    else if(data_C2 == 1){
      Firebase.setString(firebaseData,path + "/C2", "1");
    }
    old_data_C2 = data_C2;
  }
